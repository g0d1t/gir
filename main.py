...
#           ---         ---         ---         #
api_id = 13291546 # main api id from my.telegram.org/apps
api_hash = '91b08c777c23cfffd7597d472b41bcb0' # main api hash from my.telegram.org/apps
bot_token = '5642800806:AAEnIm7Wz676DMTsoIo6r_3_HmHaJ73UqyI' # main bot token from @botFather
bot_admins = [6812092996, 5938044170] # admin userID
#           ---         ---         ---         #
...
async def StopEval(client, message):
    ...
    if evalID in isWorking:
        ...
        await message.reply('<b>عملیات موردنظر یافت نشد !</b>', reply_markup=InlineKeyboardMarkup(my_keyboard), quote=True)
    ...
    if chat_id in bot_admins:
        if data == 'backToMenu':
            ...
            if myLen == 0 :
                ...
                try:
                    await bot.send_document(chat_id, './accounts.txt', caption=f'تعداد کل اکانت ها : {myLen}')
                    os.unlink('./accounts.txt')
                except:
                    pass
